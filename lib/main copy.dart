...
class _PermissionCheckScreenState extends State<PermissionCheckScreen> {
  ...
  Future<void> _checkPermissions() async {
    // Auf iOS locationWhenInUse verwenden, auf Android location
    Permission locationPermission = Platform.isIOS 
        ? Permission.locationWhenInUse 
        : Permission.location;
    
    // Erst Status prüfen
    PermissionStatus status = await locationPermission.status;
    print('📍 Initial permission status: $status');
    
    // Falls noch nicht entschieden, Berechtigung anfragen
    if (status.isDenied) {
    ...
    if (status.isGranted || status.isLimited) {
      print('📍 Permission granted! Navigating...');
      ...
      // Benutzer hat dauerhaft abgelehnt -> App-Einstellungen öffnen
      if (!mounted) return;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _showPermissionDialog();
      });
    } else {
      print('📍 Permission status: $status - showing dialog');
      // Berechtigungsdialog nach Frame zeigen
      if (!mounted) return;
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _showPermissionDialog();
      });
  ...
  void _showPermissionDialog() {
    showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'Diese App benötigt Zugriff auf den Standort.\n\n'
          'Bitte gehe zu:\nEinstellungen → Datenschutz → Ortungsdienste → Busdeskpro\n\n'
          'und wähle "Beim Verwenden der App".',
        ),
        actions: [
          TextButton(
            onPressed: () async {
          ...
          TextButton(
            onPressed: () async {
              ...
              // Erneut prüfen nach Rückkehr aus Einstellungen
              setState(() {
                _checking = true;
              });
