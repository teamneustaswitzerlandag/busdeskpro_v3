...
import 'package:bus_desk_pro/libaries/ConnectionStatusBanner.dart';
import 'package:bus_desk_pro/maps/ActionReachedDestination.dart';
...
import '../common/battery_saver_utils.dart';
import '../common/notifications/android_notifications.dart';
import '../common/notifications/ios_notifications.dart';
import '../common/notifications/notifications_manager.dart';
import '../common/utils/navigation/location_provider_interface.dart';
import '../common/utils/navigation/location_utils.dart';
import '../common/utils/navigation/position_status_listener.dart';
...
class _NavigationScreenState extends State<NavigationScreen>
  ...
  static const double _kTopBarHeight = 50;
  static const double _kBottomBarHeight = 60;
  ...
  Future<void> loadGeoRadiusSetting() async {
    ...
    setState(() {
      geoRadius = int.parse(geoRadiusSetting??'150');
    });
  }
            ...
            children: [
              ...
              Positioned(
                ...
                child: Container(
                  ...
                  color: Colors.transparent, // optionaler Hintergrund für bessere Lesbarkeit
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      // Linker Bereich mit zentriertem Text
                      Expanded(
                        child: Align(
                          alignment: Alignment.centerLeft,
                          child: Text(
                            "\n${GblStops[currentStoppIndex]["firstname"] != null ? GblStops[currentStoppIndex]["firstname"] : ''} ${GblStops[currentStoppIndex]["lastname"]} - ${GblStops[currentStoppIndex]["address"]}\n\nPlanzeit: ${GblStops[currentStoppIndex]["time"]}",
                            style: TextStyle(
                              fontSize: 18,
                        ...
                        onPressed: () async {
                          ...
                          if (result == false) {
                            ...
                            Navigator.of(context).pushAndRemoveUntil(
                              MaterialPageRoute(builder: (context) => RoutingScreen(
  ...
  void _startNavigation() {
    ...
    // CUSTOM: Speed Zoom 3
    _visualNavigator.cameraBehavior = Navigation.SpeedBasedCameraBehavior();
    setState(() {
      zoomIn = false;
      zoomOut = true;
    });
  ...
  void _setupListeners() {
    ...
    double _berechneDistanz(GeoCoordinates start, GeoCoordinates ziel) {
      const double earthRadius = 6371000; // Erd-Radius in Metern

      double lat1 = _gradZuRad(start.latitude);
      double lon1 = _gradZuRad(start.longitude);
      double lat2 = _gradZuRad(ziel.latitude);
      double lon2 = _gradZuRad(ziel.longitude);
    ...
    _visualNavigator.navigableLocationListener = Navigation.NavigableLocationListener((location) async {
      ...
      // CUSTOM: Entfernung zum Ziel prüfen
      final ziel = WayPointInfo(coordinates: GeoCoordinates(double.parse(GblStops[currentStoppIndex]['lat']), double.parse(GblStops[currentStoppIndex]['long'])));
      final currentLocation = location.originalLocation.coordinates;
      final entfernung = _berechneDistanz(currentLocation, ziel.coordinates);

      if (!zielErreicht && entfernung <= geoRadius) {
        zielErreicht = true;
        print("Ziel erreicht");
        //print("🎉 Ziel erreicht! Du bist im 50m Radius.");
        try {
          final _storage = FlutterSecureStorage();
          final response = await http.get(Uri.parse("http://bus-dashboard.dxp.azure.neusta.cloud:7698/arrivedInRadius?query=${"${GblStops[currentStoppIndex]['station_id']}_${GblStops[currentStoppIndex]['time']}_${GblStops[currentStoppIndex]['lastname']}_${GblStops[currentStoppIndex]['firstname']}_${GblStops[currentStoppIndex]['isostring']}_${TourNameGbl}_${MandantAuth}_CUR${DateFormat('HH:mm:ss').format(DateTime.now())}"}"));
          if (response.statusCode == 200) {
            ...
            bufferOfLogTimesSend.add("${"${GblStops[currentStoppIndex]['station_id']}_${GblStops[currentStoppIndex]['time']}_${GblStops[currentStoppIndex]['lastname']}_${GblStops[currentStoppIndex]['firstname']}_${GblStops[currentStoppIndex]['isostring']}_${TourNameGbl}_${MandantAuth}_CUR${DateFormat('HH:mm:ss').format(DateTime.now())}"}");
            await _storage.write(key: 'bufferOfLogTimesSend', value: bufferOfLogTimesSend.join('||'));
          } else {
            ...
            bufferOfLogTimesNotSend.add("${"${GblStops[currentStoppIndex]['station_id']}_${GblStops[currentStoppIndex]['time']}_${GblStops[currentStoppIndex]['lastname']}_${GblStops[currentStoppIndex]['firstname']}_${GblStops[currentStoppIndex]['isostring']}_${TourNameGbl}_${MandantAuth}_CUR${DateFormat('HH:mm:ss').format(DateTime.now())}"}");
            await _storage.write(key: 'bufferOfLogTimesNotSend', value: bufferOfLogTimesNotSend.join('||'));
          }
        } catch (e) {}
      }
    });
    ...
    _visualNavigator.destinationReachedListener = Navigation.DestinationReachedListener(() {
      _stopNavigation();
      // CUSTOM - Disabled, because blank Screen
  ...
  void _setupVoiceTextMessages() async {
    await _flutterTts.setLanguage("de-DE"); //en-US

    _visualNavigator.eventTextListener = Navigation.EventTextListener((Navigation.EventText eventText) async {
      if (eventText.type == Navigation.TextNotificationType.maneuver) {
        if (_soundEnabled) {
          ...
          if (maneuver == null) {
            ...
            maneuverText = maneuver.getActionText(context);
          }

          if (_currentManeuverDistance > 0) {
            maneuverText = Util.makeDistanceString(context, _currentManeuverDistance) + ' ' + maneuverText;
          }

          _flutterTts.speak(maneuverText); // eventText.text
        }
  ...
  PreferredSize? _buildTopBar(BuildContext context) {
    ...
    return PreferredSize(
        preferredSize: Size.fromHeight(_kTopBarHeight + 40),
        child: Column(
          children: [
    ...
    return Align(
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            // CUSTOM: Display Navigationspeed
...
extension _ManeuverImagePath on Routing.ManeuverAction {
  String get imagePath {
    ...
    return "lib/assets/maneuvers/$subDir/png/${toString().split(".").last}.png";
  }
}

// Neue Funktion für Android Auto Navigation zum aktuellen Ziel
void _startAndroidAutoNavigationToCurrentDestination() async {
  try {

    if(isAndroidAutoConnected == true) {
      print('=== ANDROID AUTO ZIEL BESTÄTIGT ===');
      ...
      final intent = AndroidIntent(
        ...
        flags: [
          ...
          Flag.FLAG_ACTIVITY_NO_USER_ACTION,
          Flag.FLAG_ACTIVITY_SINGLE_TOP,
        ],
      );
      
      print('Android Auto Navigation zum Ziel wird gestartet...');
      await intent.launch();
      print('Android Auto Navigation erfolgreich gestartet!');
...
// Funktion um App wieder in den Vordergrund zu bringen
