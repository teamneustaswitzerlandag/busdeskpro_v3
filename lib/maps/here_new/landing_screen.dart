...
import 'common/extensions/error_handling/map_loader_error_extension.dart';
import 'common/file_utility.dart';
...
class _LandingScreenState extends State<LandingScreen> with Positioning, WidgetsBindingObserver {
  ...
  void _onMapCreated2(HereMapController hereMapController) {
  ...
  void _resetCurrentPosition() {
    GeoCoordinates coordinates = lastKnownLocation!.coordinates;
  ...
  void _updateConsentState(PositioningEngine positioningEngine) async {
    final _storage = FlutterSecureStorage();
    await _storage.write(key: 'HERESDKConsent', value: 'true');
    setState(() => _consentState = positioningEngine.userConsentState);
    double lat = double.parse(GblStops[currentStoppIndex]['lat']);
    double lng = double.parse(GblStops[currentStoppIndex]['long']);
