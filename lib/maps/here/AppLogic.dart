...
import 'package:bus_desk_pro/maps/here/RouteCalculator.dart';
import 'package:bus_desk_pro/maps/here/time_utils.dart';
import 'package:here_sdk/search.dart';

import 'NavigationExample.dart';
import 'RouteCalculator.dart';
...
class AppLogic {
    ...
    _hereMapController.gestures.longPressListener = LongPressListener((gestureState, touchPoint) {
      ...
      if (gestureState == GestureState.begin) {
        if (_setLongPressDestination) {
          ...
          _addCircleMapMarker(_destinationWaypoint!.coordinates, "lib/assets/green_dot.png");
          _updateMessageState("New long press destination set.");
        } else {
          _startWaypoint = HERE.Waypoint(geoCoordinates);
          _addCircleMapMarker(_startWaypoint!.coordinates, "lib/assets/green_dot.png");
  ...
  void startNavigation(double _destLat, double _destLong, String address) async {
    Location? location = _navigationExample.getLastKnownLocation();
    _startWaypoint = HERE.Waypoint(location!.coordinates);
    //_startWaypoint = HERE.Waypoint(GeoCoordinates(53.061188, 8.6640296));
    /*final url = Uri.parse("https://geocode.search.hereapi.com/v1/geocode?q=Wiesengrund 10, 27751 Delmenhorst&apikey=fpil72DCnrt86RtRWgBxAdLu1XnG0EQVR8pHLS_K2LU");
    final response = await http.get(url);
    if (response.statusCode == 200) {
      var data = json.decode(response.body);
      if (data["items"].length > 0) {
        GeoCoordinates centerGeoCoordinates = _hereMapController.camera.state.targetCoordinates;
        _destinationWaypoint = HERE.Waypoint(GeoCoordinates(data["items"][0]["position"]["lat"], data["items"][0]["position"]["lng"]));
      }
    } else {
